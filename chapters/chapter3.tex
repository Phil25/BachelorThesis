...
The path must be unique to server installation.
For convenience purposes, it is defined to be relative to SourceMod installation root.
From here there are the shortest paths to commonly used directories, for instance, \textbf{mod/addons/sourcemod/plugins}.

Relative paths are supported, such as "\verb|..|" to go up a directory.
This is required because, as mentioned previously, addon files are not limited to the SourceMod directories.
...
This must be accounted for and limited to the server installation, such as \textbf{mod/}.
