...
The \textit{Filter...} field is used to dynamically filter self submitted addons in either section by entering parts of their name.
...
Selecting a mode is simply typing its name after the command, followed by a potential list of addons passed as space-separated arguments: \verb|smam install rtd|.
...
After the \verb|install| keyword addons IDs should be provided as space-separated arguments.
...
Luckily, the game server has a specified structure which can easily be detected, which allows for simple but efficient heuristics to be implemented.
...
Users are also able to specify the path themselves using the \verb|-d [--destination]| flag, using which same method will be applied to find the correct folder.
All the operations henceforth will be performed in relation to SourceMod's root directory.
