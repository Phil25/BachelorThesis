...
LimeTech \cite{limetech} is not used by the public but rather a single developer, who also is a major contributor to both SourceMod and SourcePawn.
The website is his portfolio but additionally hosts files for many popular extensions, making it worthwhile to consider when finding the way to standardize distribution.
...
This has affected the No Thriller Taunt plugin \cite{alliedmodders-no-thriller}.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Files} -- Paths and filenames defining a plugin; for installation.
    \item \textbf{Games} -- Multiple choice field defining which games the plugin works for, may be all; for searching.
    \item \textbf{Dependencies} -- Names of other plugins which this one is dependent upon; for installation.
\end{enumerate}

\subsection{Fields utilized for searching}

Optional but equally important aspects of the database are its search capabilities.
...
If so, there will be no need to store the author, description or category, as all three could be fetched from the forums' database.
...
It is by this field, that the website will be detected by the client-side application, after which the appropriate method of parsing will be used.
...
This is the field the client uses to link attachments parsed from the \textit{base URL} to their actual placement and file names, as they appear on the disk.
...
This is the property that relates directly to where the file belongs on the disk, it must be unique per server installation.
For convenience purposes, it is defined to be relative to SourceMod installation root.
From there, are the shortest paths to commonly used directories, for instance, \path{mod/addons/sourcemod/plugins} needs only to be \path{./plugins}.
...
The actual name itself is not prone to change, however, there may be situations when a developer includes a version number.
This happens most often when a whole project is archived but may also come up when the plugin is a single binary file.
    For this purpose, the support for regex-defined file identifiers is included.
...
In practice, it is quite rare for a plugin to have dependency on another.
...
The database is set up on a virtual private server provided by \textit{Digital Ocean} \cite{digital-ocean}.
Interaction is achieved through a web server bound to a custom domain name -- \path{smamdb.net}.
Communication with that server is end-to-end encrypted with TLS using a certificate issued by \textit{Let's Encrypt} \cite{lets-encrypt}.
This warrants three important aspects of fetching packages \cite{ibm-tls-guarantees}:
\begin{enumerate}
...
One reason could be that the developer who is publishing their work fancies distributing it on their own website, which happens to be not secured properly.
Another reason could be that the attacker had gained access to publishing or editing plugins, rendering them compromised.

By using the OAuth2 \cite{valve-oauth2} protocol, Valve provides third-party authentication mechanism against their Steam services.
It is by these mechanisms that publishers are recognized and validated, as each of them is expected to have a Steam account.
...
The burden of implementation is placed on the client application, it is where the complexity of the system is located.

The website is a completely different matter.
The custom interface exists for human interaction and is not needed by the client.
...
This makes it possible to write a simple solution for a self hosted web server.
...
To support this, the application is built with the help of a powerful, cross-platform build system utilizing CMake \cite{cmake} and Conan \cite{conan} tooling.
CMake is a build system generator which creates and configures files for the selected build utility, such as UNIX Makefiles or Visual Studio.
...
The dependencies are downloaded to a remote location and built if the specific pre-compiled binaries are not available, provided they even need to be compiled.
The appropriate paths are then given to the selected generator, such as CMake, to aid the compilation process in finding and linking the libraries.
...
It is a boolean property which specifies whether the user requested a particular addon themselves or it was installed as a dependency outside user's instructions.
...
\begin{enumerate}
\item
Placing a period at the beginning of the file in UNIX-based file systems effectively hides it.
...
\begin{enumerate}
    ...
    \item \verb|remove| -- remove downloaded files and clean up implicitly installed dependencies,
...
\item
The plugin must not be required by any other one, i.e., the number of plugins in need of that dependency, excluding itself, must equal one.
...
\item
The user has not passed the \verb|--no-deps| flag, which forces the application not to delete any dependencies.
...
The reasons are that those features are either impossible to implement, or the benefits are simply not worth the trade-off for complexity and development time.
...
Version management gives the flexibility of installing a specific version of any package, often being the go-to solution.
